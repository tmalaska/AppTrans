package com.cloudera.sa.apptrans.etl.machinelearning.kudu

import java.util.{Calendar, Date}

import com.cloudera.sa.apptrans.model._
...
object MlLibOnKudu {
  def main(args: Array[String]): Unit = {
    ...
    val vectorRDD:RDD[Vector] = sqlContext.sql("select * from account_mart").map(r => {
      val accountMart = AccountMartBuilder.build(r)
      generateVectorOnly(accountMart)
    })
  ...
  def generateVectorOnly(accountMart: AccountMart): Vector = {
    ...
    val array = Array(accountMart.purchaseTotal,
      ...
      accountMart.paymentDebitTotal.toDouble,
      accountMart.paymentPaypalCount.toDouble,
      accountMart.paymentPaypalTotal.toDouble)
    Vectors.dense(array)
  }
/*
